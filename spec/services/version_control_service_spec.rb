  ...
  describe '.checkout' do
    ...
    context 'given a to_dir' do
      it 'clones the given repo to the given dir' do
        expect(GitAdapter).to receive(:clone_repo).with(repo_url: anything, to_dir: '/my/dir')
        described_class.checkout(repo_url: 'myrepourl', to_dir: '/my/dir')
      end

      it 'checks out the given ref in the given dir' do
        expect(GitAdapter).to receive(:checkout).with(ref: 'myref', dir: '/my/dir')
        described_class.checkout(repo_url: 'myrepourl', ref: 'myref', to_dir: '/my/dir')
      end
    ...
    context 'given no to_dir' do
      ...
      it 'clones the given repo to a tmpdir' do
        expect(GitAdapter).to receive(:clone_repo).with(repo_url: 'myrepourl', to_dir: '/tmp/dir')
        described_class.checkout(repo_url: 'myrepourl')
      end

      it 'checks out the given ref in the tmpdir' do
        expect(GitAdapter).to receive(:checkout).with(ref: 'myref', dir: '/tmp/dir')
        described_class.checkout(repo_url: 'myrepourl', ref: 'myref')
      end
    ...
    it 'returns the current commit' do
      expect(described_class.checkout(repo_url: 'myrepourl')).to eq('my current commit')
    end
  end
end
